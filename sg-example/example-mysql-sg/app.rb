...
post '/providers' do
  ...
  provider['name']         = "mysql-server-#{hostname}"
  provider['description']  = "MYSQL Server at #{hostname}"
...
post '/services' do
  ...
  service['name']         = "mysql-db-#{db[:db]}"
  service['description']  = "Mysql DB at #{hostname}/#{db[:db]}"
...
post '/bindings' do
  ...
  binding = {
    ...
    'name'            => "mysql-binding-to-#{service['name']}",
    'service_id'      => service['id'],
    'url'             => "#{provider['params']['url']}/#{service['params']['db']}",
    'protocol'        => { 'scheme' => 'mysql'}
  }
  @@bindings << binding
  binding.to_json
end
